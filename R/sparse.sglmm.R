...
sparse.sglmm.fit.binomial = function(Y, X, A, M, family, beta.start, V, offset, tol, minit, maxit, sigma.s)
{
    ...
    for (j in 1:iter)
    {
	    p_ = linkinv(offset + X %*% beta[j, ] + M %*% gamma[j, ])
	    v[j] = -2 * sum(dbinom(Y, 1, p_, log = TRUE))
    }
    D.bar = bm(v)$est
    pD = D.bar + 2 * sum(dbinom(Y, 1, p.hat, log = TRUE))
    dic = D.bar + pD
...
sparse.sglmm.fit.gaussian = function(Y, X, A, M, beta.start, offset, tol, minit, maxit, hyper)
{
	iterations = minit
	n = length(Y)
	Q = t(M) %*% (diag(rowSums(A), n) - A) %*% M
...
sparse.sglmm.fit.poisson = function(Y, X, A, M, family, beta.start, V, offset, tol, minit, maxit, tune, hyper)
{
	iterations = minit
	n = length(Y)
	Q = t(M) %*% (diag(rowSums(A), n) - A) %*% M
    ...
    linear.predictors = offset + X %*% coefficients + M %*% gamma.est
    if (hetero)
        linear.predictors = linear.predictors + M %*% delta.est
    iter = length(tau.s)
    lambda.hat = linkinv(linear.predictors)
    v = numeric(iter)
    for (j in 1:iter)
    {
    	eta_ = offset + X %*% beta[j, ] + M %*% gamma[j, ]
    	if (hetero)
    	    eta_ = eta_ + M %*% delta[j, ]
	    lambda_ = linkinv(eta_)
	    v[j] = -2 * sum(dpois(Y, lambda_, log = TRUE))
    }
    D.bar = bm(v)$est
    pD = D.bar + 2 * sum(dpois(Y, lambda.hat, log = TRUE))
    dic = D.bar + pD
    fitted.values = linkinv(linear.predictors)
    residuals = Y - fitted.values
    beta.accept = sum(diff(beta[, 1]) != 0) / iter
...
#' @param hyper (where relevant) a list containing \code{a.h} and \code{b.h}, the parameters of the gamma prior for \eqn{\tau_h}.
                        ...
                        tune = list(), hyper = list(), model = TRUE, x = FALSE, y = FALSE)
{
	...
	if (missing(A))
		stop("You must supply an adjacency matrix.")
    ...
    if (! is.numeric(maxit) || ! is.wholenumber(maxit) || maxit < 1)
        stop("'maxit' must be a positive whole number.")
    if (! is.list(tune))
        stop("'tune' must be a list.")
    if (family$family != "gaussian" && is.null(tune$sigma.s))
        stop("You must supply 'sigma.s', the proposal standard deviation for the spatial random effects.")
    if (! is.list(hyper))
        stop("'hyper' must be a list.")
    if (family$family == "gaussian" && (is.null(hyper$a.h) || is.null(hyper$b.h)))
        stop("You must supply 'a.h' and 'b.h', the hyperparameters associated with 'tau.h'.")
    if (family$family == "poisson" && ! is.null(tune$sigma.h) && (is.null(hyper$a.h) || is.null(hyper$b.h)))
        stop("You must supply 'a.h' and 'b.h', the hyperparameters associated with 'tau.h'.")
	...
	if (! is.wholenumber(q) || q < 1 || q > maxq)
	...
	fit = if (family$family == "binomial")
	          ...
	          sparse.sglmm.fit.gaussian(Y, X, A, M, nonspat$coef, offset, tol, minit, maxit, hyper)
	      else sparse.sglmm.fit.poisson(Y, X, A, M, family, nonspat$coef, V, offset, tol, minit, maxit, tune, hyper)
...
#'
#' @details This function displays (1) the call to \code{\link{sparse.sglmm}}, (2) a table of estimates, (3) the DIC value for the fit, and (4) the size of the posterior sample. Each row of the table of estimates shows the estimated regression coefficient, the \eqn{(1-\alpha)100\%}{(1-\alpha)100\%} HPD interval for the coefficient, and the Monte Carlo standard error.
#' @param object an object of class \code{sparse.sglmm}, typically the result of a call to \code{\link{sparse.sglmm}}.
